...
```bash
git clone https://github.com/omar99elnemr/AWS-3-Tier-Architecture-app.git
...
Create an S3 bucket to store your application code. Upload the `application-code/web-tier/` and `application-code/app-tier/` contents, along with the `nginx.conf` and `nginx-Without-SSL.conf` files, to a designated folder in your S3 bucket (e.g., `s3://<S3 Bucket Name>/application-code/`).
...
```bash
aws s3 cp s3://<S3 Bucket Name>/application-code/web-tier/ web-tier --recursive
cd web-tier
npm install
npm run build
```
...
cd /etc/nginx
sudo rm nginx.conf
sudo aws s3 cp s3://<S3 Bucket Name>/application-code/nginx.conf . # Copies the nginx.conf with internal LB DNS
...
2. Both Load Balancers (External and Internal)
3. Both Target Groups (TGs)
4. Both AMIs (Amazon Machine Images)
5. Both Snapshots of AMIs
6. RDS Database
7. S3 Bucket
8. SSL Certificate
9. Route 53 record
10. NAT Gateway
11. Elastic IP (associated with NAT Gateway)
12. VPC
